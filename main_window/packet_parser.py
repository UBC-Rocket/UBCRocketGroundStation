...
class PacketParser:
    ...
    def state(self, byte_stream: BytesIO, header: Header):
        data = {}
        state_bytes = byte_stream.read(2)
        state_int = self.bytestoint(state_bytes)
        data_entry_value = STATE_IDS[state_int]
        data[DataEntryIds.STATE] = data_entry_value

        LOGGER.info("New state: " + str(data_entry_value.name))
