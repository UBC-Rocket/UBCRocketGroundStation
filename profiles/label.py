...
def update_altitude(rocket_data: RocketData, device: DeviceType) -> str:
    return str(rocket_data.last_value_by_device(device, DataEntryIds.CALCULATED_ALTITUDE.value))
...
def update_gps(rocket_data: RocketData, device: DeviceType) -> str:
    latitude = rocket_data.last_value_by_device(device, DataEntryIds.LATITUDE.value)
    longitude = rocket_data.last_value_by_device(device, DataEntryIds.LONGITUDE.value)
    return str(latitude) + ", " + str(longitude)


def update_state(rocket_data: RocketData, device: DeviceType) -> str:
    return str(rocket_data.last_value_by_device(device, DataEntryIds.STATE.value))


def update_pressure(rocket_data: RocketData, device: DeviceType) -> str:
    return str(rocket_data.last_value_by_device(device, DataEntryIds.PRESSURE.value))
...
def update_acceleration(rocket_data: RocketData, device: DeviceType) -> str:
    ...
    accel = math.sqrt(
        nonezero(rocket_data.last_value_by_device(device, DataEntryIds.ACCELERATION_X.value)) ** 2
        + nonezero(rocket_data.last_value_by_device(device, DataEntryIds.ACCELERATION_Y.value)) ** 2
        + nonezero(rocket_data.last_value_by_device(device, DataEntryIds.ACCELERATION_Z.value)) ** 2
    )
    return str(accel)
